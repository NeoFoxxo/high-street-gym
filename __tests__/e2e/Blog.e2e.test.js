...
describe("Blog page functionality", () => {
  ...
  it("Create blog post button should not be visable if not authenticated", async () => {
    await driver.findElement(By.linkText("My Workout Routine")).click();
    await driver.wait(until.urlIs("http://localhost:3000/blog-details/29"), 2000);
    const currentUrl = await driver.getCurrentUrl();
    expect(currentUrl).toBe("http://localhost:3000/blog-details/29");
...
describe("Create blog post functionality", () => {
  ...
  beforeEach(async () => {
    ...
    await driver.wait(until.urlIs("http://localhost:3000/create-blog"), 2000);
  });
  ...
  it("Should display error if no credentials are provided", async () => {
    await driver.findElement(By.xpath("//button[@type='submit']")).click();
    const titleError = await driver.findElement(By.xpath("//*[contains(text(),'Title Required')]")).getText();
    const articleError = await driver.findElement(By.xpath("//*[contains(text(),'Article Required')]")).getText();
    expect(titleError).toBe("Title Required");
    expect(articleError).toBe("Article Required");
  })
